...
client.on(discord_js_1.Events.InteractionCreate, async (interaction) => {
    ...
    if (interaction.commandName === command.name) {
        ...
        await interaction.reply(`Sent message to ${users.length} users: \`\`\`${source.content}\`\`\``);
    }
});
// Deploy slash command
const rest = new discord_js_2.REST({ version: "9" }).setToken(process.env.TOKEN);
...
(async () => {
    try {
        ...
        await rest.put(discord_js_2.Routes.applicationGuildCommands(clientId, guildId), {
            body: [command.toJSON()],
        });
...
client.on(discord_js_1.Events.MessageUpdate, async (oldMessage, newMessage) => {
    ...
    await newMessage.reply(`Updated synced message to ${syncedMessages.length} users: \`\`\`${newMessage.content}\`\`\``);
});
